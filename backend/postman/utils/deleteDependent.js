...
const deleteMove = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(Move,filter);
    return response;
  } catch (error){
    throw new Error(error.message);
  }
...
const deletePiece = async (filter) =>{
  try {
    ...
    if (piece && piece.length){
      ...
      const moveFilter = { $or: [{ pieceId : { $in : piece } }] };
      const moveCnt = await dbService.deleteMany(Move,moveFilter);
...
const deleteSlot = async (filter) =>{
  try {
    let slot = await dbService.findMany(Slot,filter);
    if (slot && slot.length){
      slot = slot.map((obj) => obj.id);

      const moveFilter = { $or: [{ targetPosition : { $in : slot } }] };
      const moveCnt = await dbService.deleteMany(Move,moveFilter);

      let deleted  = await dbService.deleteMany(Slot,filter);
      let response = { move :moveCnt, };
      return response; 
    } else {
      return {  slot : 0 };
    }
...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const moveFilter = { $or: [{ addedBy : { $in : user } }] };
      const moveCnt = await dbService.deleteMany(Move,moveFilter);
...
const countMove = async (filter) =>{
  try {
    const moveCnt =  await dbService.count(Move,filter);
    return { move : moveCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const countPiece = async (filter) =>{
  try {
    ...
    if (piece && piece.length){
      ...
      const moveFilter = { $or: [{ pieceId : { $in : piece } }] };
      const moveCnt =  await dbService.count(Move,moveFilter);
...
const softDeleteMove = async (filter,updateBody) =>{  
  try {
    const moveCnt =  await dbService.updateMany(Move,filter);
    return { move : moveCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeletePiece = async (filter,updateBody) =>{  
  try {
    ...
    if (piece.length){
      ...
      const moveFilter = { '$or': [{ pieceId : { '$in' : piece } }] };
      const moveCnt = await dbService.updateMany(Move,moveFilter,updateBody);
...
const softDeleteSlot = async (filter,updateBody) =>{  
  try {
    let slot = await dbService.findMany(Slot,filter, { id:1 });
    if (slot.length){
      slot = slot.map((obj) => obj.id);

      const moveFilter = { '$or': [{ targetPosition : { '$in' : slot } }] };
      const moveCnt = await dbService.updateMany(Move,moveFilter,updateBody);
      let updated = await dbService.updateMany(Slot,filter,updateBody);
